...
export default function Films() {
    // data đầy đủ nhất lấy từ API về
    ...
    // fetch lại data trong trường hợp user vào thẳng admin ko qua trang chủ
    useEffect(() => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, film) => {
                return <Fragment>
                    <NavLink key={1} className=" mr-2  text-2xl" to={`/admin/films/edit/${film.maPhim}`}><EditOutlined style={{ color: 'blue' }} title="Edit" /></NavLink>
                    ...
                    }}><DeleteOutlined style={{ color: 'red' }} title="Delete" /> </span>

                    <NavLink key={3} className=" mr-2 text-2xl" to={`/admin/films/showtime/${film.maPhim}/${film.tenPhim}`} onClick={() => {
                        localStorage.setItem('filmParams', JSON.stringify(film));
                    }}><CalendarOutlined style={{ color: 'green' }} title="Showtime" /> </NavLink>
                </Fragment>
            },
            sortDirections: ['descend', 'ascend'],
            width: '25%'
    ...
    return (
        <div>
            <h3 className="text-4xl">Film Management</h3>
